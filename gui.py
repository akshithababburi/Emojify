...
def show_vid():
    ...
    if flag1 is None:
        ...
        show_vid2(c)
        lmain.after(3000, show_vid)
    if cv2.waitKey(1) & 0xFF == ord('q'):
        exit()
    

def show_vid2(maxindex = 0):
    print("+++here++"+ emoji_dist[maxindex])
    frame2=cv2.imread(emoji_dist[maxindex])
    print(np.max(frame2))
    pic2=cv2.cvtColor(frame2 ,cv2.COLOR_BGR2RGB)
    img2=Image.fromarray(pic2)
    imgtk2=ImageTk.PhotoImage(image=img2)
    lmain2.imgtk2=imgtk2
    lmain3.configure(text=emotion_dict[maxindex],font=('arial',45,'bold'))
